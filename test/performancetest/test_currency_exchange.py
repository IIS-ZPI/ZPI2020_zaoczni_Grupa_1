...
class TestCurrencyExchangeDataParser(TestCase):
    ...
    def __performance_run(self, amount_of_days, method):
        ...
        session_data_EUR["rates"] = session_data_EUR['rates'][:amount_of_days]
        session_data_USD = valid_test_USD_data["dataset"].copy()
        session_data_USD["rates"] = session_data_USD['rates'][:amount_of_days]
    ...
    def test_parse_currency_exchange_performance(self):
        ...
        for amount_of_days in range(1, min(len(valid_test_USD_data["dataset"]["rates"]),
                                    len(valid_test_EUR_data["dataset"]["rates"])) + 1):
            ...
            self.assertTrue(
                result < amount_of_days * TestCurrencyExchangeDataParser.threshold * 5
                if amount_of_days == 1
                else result < amount_of_days * TestCurrencyExchangeDataParser.threshold
            )
